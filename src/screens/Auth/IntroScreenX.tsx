...
import { COLORS, GRADIENTS, FONTS, SHADOWS, SPRING } from '../../constants/theme';
...
// ─── Sonar grid ───────────────────────────────────────────────────────────────
function SonarGrid({ opacity }: SonarGridProps) {
  const rings = [60, 110, 165, 220, 280];
  return (
    <Animated.View style={[StyleSheet.absoluteFill, { opacity }, { pointerEvents: 'none' } as any]}>
      {rings.map((r) => (
        <View key={r} style={{
          position: 'absolute',
          left: CX - r, top: CY - r,
...
// ─── Sonar sweep arm ──────────────────────────────────────────────────────────
function SonarSweep({ visible }: SonarSweepProps) {
  const rot = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    if (!visible) return;
    Animated.loop(
      Animated.timing(rot, { toValue: 1, duration: 3000, easing: Easing.linear, useNativeDriver: Platform.OS !== 'web' })
    ).start();
  }, [visible, rot]);
  ...
  return (
    <Animated.View style={[styles.sweepContainer, { transform: [{ rotate }], pointerEvents: 'none' } as any]}>
...
// ─── Hex logo ─────────────────────────────────────────────────────────────────
function HexLogo({ progress }: HexLogoProps) {
  ...
  const hexCorners = (r: number) => [...Array(6)].map((_, i) => {
    const a = (Math.PI / 180) * (60 * i - 30);
    return { x: r * Math.cos(a) - 3, y: r * Math.sin(a) - 3 };
  });

  return (
    <Animated.View style={[{ width: RING3 * 2 + 16, height: RING3 * 2 + 16, alignItems: 'center', justifyContent: 'center' }, { transform: [{ scale: scaleV }] } as any]}>
      <Animated.View style={[{
        position: 'absolute', width: RING3 * 2 + 40, height: RING3 * 2 + 40,
        borderRadius: RING3 + 20, backgroundColor: 'rgba(59, 130, 246, 0.06)',
        ...(SHADOWS.glow(COLORS.primary, 30, 0.5) as ViewStyle),
      }, { opacity: glowOp }]} />
      ...
      <Animated.View style={[{
        ...
        ...(SHADOWS.glow(COLORS.primary, 12, 0.8) as ViewStyle),
      }, { opacity: ring1Op }]}>
      ...
      {hexCorners(RING2).map((pos, i) => (
        <Animated.View key={i} style={[{
          ...
          ...(SHADOWS.glow(COLORS.primary, 4, 1) as ViewStyle),
        }, { opacity: ring2Op }]} />
      ))}
    </Animated.View>
  );
...
// ─── Wave bar ─────────────────────────────────────────────────────────────────
function WaveBar({ width: w, delay }: WaveBarProps) {
  return (
    <MotiView
      from={{ opacity: 0.4 }}
      animate={{ opacity: 1 }}
...
// ─── Glitch title ─────────────────────────────────────────────────────────────
function GlitchTitle({ show }: GlitchTitleProps) {
  if (!show) return null;
...
// ─── Metric chip ──────────────────────────────────────────────────────────────
function MetricChip({ label, value, unit, color, index }: MetricChipProps) {
  return (
    <MotiView
      from={{ translateX: -50, opacity: 0 }}
      animate={{ translateX: 0, opacity: 1 }}
      transition={{ type: 'spring', delay: 2500 + index * 150, ...SPRING.gentle } as any}
    >
...
// ─── Progress bar ─────────────────────────────────────────────────────────────
function WaterProgress({ show }: WaterProgressProps) {
  return (
    <View style={styles.progressOuter}>
      ...
      <View style={styles.progressTrack}>
        <MotiView
          ...
          transition={{ delay: 3000, duration: 1500, type: 'timing', easing: Easing.bezier(0.25, 0.46, 0.45, 0.94) } as any}
...
// ─── Particle ─────────────────────────────────────────────────────────────────
function DataParticle({ x, size, duration, delay, baseOp }: DataParticleProps) {
  return (
    <MotiView
      from={{ translateY: height * 0.8, opacity: 0 }}
      animate={{ translateY: height * 0.1, opacity: [0, baseOp, baseOp, 0] } as any}
      transition={{
        loop: true,
        duration,
        delay,
      ...
      style={[styles.particle, {
        ...
        ...(SHADOWS.glow(COLORS.primary, 3, 0.8) as ViewStyle),
        pointerEvents: 'none'
      } as any]}
    />
  );
...
export default function IntroScreen(props: IntroScreenProps) {
  ...
  return (
    <Animated.View style={[styles.root, { opacity: screenOp, transform: [{ translateY: screenY }] } as any]}>
        ...
        <AnimatePresence>
          {tickOn && (
            <MotiView
              ...
              transition={{ delay: 200, duration: 600 } as any}
      ...
      {barOn && <WaterProgress show={barOn} />}
...
const styles = StyleSheet.create({
  ...
  sweepArm: {
    ...
    ...(SHADOWS.glow(COLORS.primary, 8, 0.9) as ViewStyle),
  },
  sweepTrail: {
    position: 'absolute', left: 280, top: 280,
    width: 280, height: 1.5,
  ...
  titleReal: {
    ...
    ...Platform.select({
      web: { textShadow: `0 0 20px ${COLORS.primary}40` },
      default: { textShadowColor: COLORS.primary + '40', textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 20 },
    }),
  },
  ...
  progressFill: {
    ...
    ...(SHADOWS.glow(COLORS.primary, 6, 1) as ViewStyle),
  },
  progressEdge: {
