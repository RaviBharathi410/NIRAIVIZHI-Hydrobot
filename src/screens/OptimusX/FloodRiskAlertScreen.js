...
export default function FloodRiskAlertScreen() {
    const [riskLevel, setRiskLevel] = useState(25); // 0-100

    const getRiskStatus = () => {
        if (riskLevel < 35) return { label: 'LOW', color: COLORS.success, icon: 'check-circle' };
        if (riskLevel < 70) return { label: 'MODERATE', color: COLORS.warning, icon: 'alert-circle' };
        return { label: 'CRITICAL', color: COLORS.danger, icon: 'alert-octagon' };
    };
